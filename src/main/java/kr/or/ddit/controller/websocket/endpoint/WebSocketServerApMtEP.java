...
public class WebSocketServerApMtEP {
	...
	public void onOpen(Session webSocketSession) throws Exception {
		...
		try {
			mtInfo = (ApmeetingVO) httpSession.getAttribute("APMEET_INFO");
			memberInfo = (MemberVO) httpSession.getAttribute("USER_LOGININFO");
			
			String mt_num = mtInfo.getApmt_num();
	...
	public void onMessage(byte[] imgBytes) {
		Map<String ,Session> memList = sessionList.get(mtInfo.getApmt_num());
	...
	public void onClose(Session session) {
		try {
			...
			sessionList.get(mtInfo.getApmt_num()).remove(memberInfo.getMem_id());
			session.close();
		} catch (IOException e) {
			e.printStackTrace();
		}
