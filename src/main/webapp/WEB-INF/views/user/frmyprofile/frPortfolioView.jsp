...
$(function(){
	...
	var str = "${portInfo.po_start}";
	var po_start_year = str.substring(0,4);
	var po_start_month = str.substring(5,7);
	var str2 = "${portInfo.po_end}";
	var po_end_year = str2.substring(0,4);
	var po_end_month = str2.substring(5,7);
	var po_cate = '${portInfo.po_cate_num}';
	var cate_category = '${cateInfo.cate_category}'
	
	if(cate_category == '1'){
		$('#po_cate').val('1');
		var option2 = "<option selected='selected' value=''>세부 카테고리</option>";
		 option2 += "<option value='10'>웹</option>"+
		...
		$('#po_cate_num').val(po_cate);
	}else{
		$('#po_cate').val('0');
		var	option1 = "<option selected='selected' value=''>세부 카테고리</option>";
		option1 += "<option value='1'>웹</option>"+
			"<option value='2'>어플리케이션</option>"+
			"<option value='3'>워드프레스</option>"+
			"<option value='4'>퍼블리싱</option>"+
			"<option value='5'>일반 소프트웨어</option>"+
...
// 	$('input[name=po_title]').val('${portInfo.po_title}');
	$('textArea[name=po_content]').val('${portInfo.po_content}');
	
	$('select[name=po_start_year]').val(po_start_year);
	$('select[name=po_start_month]').val(po_start_month);
	$('select[name=po_end_year]').val(po_end_year);
	$('select[name=po_end_month]').val(po_end_month);
	$('input[name=po_percent]').val('${portInfo.po_percent}');
	$('select[name=part_num]').val('${portInfo.part_num}');
	$('input[name=po_title]').val('${portInfo.po_title}');
	$('input[name=lang_tech]').val('${portInfo.po_tech}');
	if(eval('${empty portFileInfo.pofile_file_name}')){
		$('#portFile').val('파일없음');
	}else{
		$('#portFile').val('${portFileInfo.pofile_file_name}');
	}
	$('input[name=po_explainfile]').val('${portInfo.po_explainfile}');
	$('select[name=po_tech]').val('${portInfo.po_tech}');
...
//	 	 		matchContains: true
	...
	$('#btnSave').click(function(){
		...
		if(po_start2>po_end2){
			swal("경고","퇴사일이 입사일보다 빠를수 없습니다.", "error"); return false; return false;
		}
		...
		swal({
		    ...
		    .then(function(result){
					if (result){
						...
						$('#portFolioForm').append('<input type="hidden" name="po_num" value="' + po_num + '">');
	...
	$('#fileDelete').click(function(){
		$.ajax({
	         type:'POST',
	         dataType:'JSON',
	         data:{pofile_num : '${portFileInfo.pofile_num}',
	        	 pofile_save_name : '${portFileInfo.pofile_save_name}'},
	         url:'${pageContext.request.contextPath}/freProfile/portFileDelete.do',
	         error:function(result){
	            alert(result.message);
	         },
	         success:function(result){
	            if(eval(result.result.flag)){
	              $('#portFile').attr('type', 'hidden');
	              $('#fileDelete').attr('type', 'hidden');
	              $('#fileForm').append('<input type="file" name="portFile">')
	            }else{
	            }
	         }
	      });
			...
			<div class="block-content">
				<div class="row justify-content-center py-20">
					<div class="col-xl-7">
						...
						<form id="portFolioForm" class="js-validation-bootstrap" action='${pageContext.request.contextPath }/freProfile/updatePortfolio.do' method="post" enctype="multipart/form-data">
							...
							<div class="form-group row" id="select_div">
								...
								<div class="col-lg-5">
                                                <select class="form-control" id="po_cate" name="po_cate">
							...
							<div class="form-group row">
                                            <label class="col-lg-2 col-form-label" for="val-username">프로젝트 시작일
												<span class="text-danger">*</span>
											</label>
                                            <div class="col-lg-5">
							...
							<div class="form-group row">
                                            <label class="col-lg-2 col-form-label" for="val-username">프로젝트 종료일
												<span class="text-danger">*</span>
											</label>
                                            <div class="col-lg-5">
							...
							<div class="form-group row">
			                    ...
			                    <c:if test="${not empty portFileInfo.pofile_file_name}">
                                <input type="text" id="portFile" disabled="disabled">
				                    <input type="button" id="fileDelete" class="btn btn-secondary mr-5 mb-5" value="삭제">
				                    </c:if>
				                    <c:if test="${empty portFileInfo.pofile_file_name}">
...
<!-- 							</div> -->
							<div class="form-group row">
                                            <label class="col-lg-2 col-form-label" for="val-username">관련 기술
												<span class="text-danger">*</span>
											</label>
											<div class="col-lg-10">
                            ...
                            <div class="form-group">
                                       ...
                                       <button type="button" class="btn btn-alt-primary" id="btnSave" name="btnSave">수정 완료</button>
                   			</div>
						</form>
					</div>
