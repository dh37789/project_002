...
$(function(){
	...
	var cols = [{ data: 'no', type: 'text', width: 60},
                { data: 'id', type: 'text', width: 100},
                { data: 'requestId', type: 'text', width: 100},
                { data: 'content', type: 'text', width: 500},
                { data: 'sort', type: 'text'},
                { data: 'start', type: 'text'},
                { data: 'end', type: 'text'},
                { data: 'person', type: 'text', width: 100},
                { data: 'relation', type: 'text', width: 120},
                { data: 'time', type: 'text'}];
        ...
        var hTable = new Handsontable(table, {
            ...
            columns: cols,
            rowHeaders: true,
            colHeaders: ['No','단위업무ID', '요구사항ID', '단위업무명', '분류', '업무개시일',
                         '업무종료일', '담당자','연관 단위업무' ,'예상시간수'],
            colWidths: 120,
            rowHeights: 23,
            autoRowSize: false,
            autoColSize: true,
...
<h3 class="text-white" style="padding-left: 20px;">단위 업무 정의서</h3>&nbsp;<small class="text-white">최종 수정 날짜 : --:--:--  최종 수정자 : ---</small>
</c:if>
<c:if test="${not empty tableInfo }">
<h3 class="text-white" style="padding-left: 20px;">단위 업무 정의서</h3>&nbsp;<small class="text-white">최종 수정 날짜 : ${tableInfo.data_date }  최종 수정자 : ${tableInfo.mem_id }</small>
</c:if>
