...
<script type="text/javascript">
	$(function() {
		...
		if ('${category.cate_num}' >= 10) {
			$('#category').val('1');
		}else if('${category.cate_num}' == ''){
		}else{
			$('#category').val('0');
		}
		$('input[name=pr_title]').val("${project.pr_title}");
		$('input[name=pr_ex_period]').val("${project.pr_ex_period}");
		$('input[name=pr_payment]').val("${project.pr_payment}");
		$('input[name=pr_type]:input[value="${project.pr_type}"]').attr('checked',true);
		$('input[name=pr_add]').val("${project.pr_add}");
		
		$('input[name=pr_deadline]').val("${fn:substring(project.pr_deadline,0,10)}");
		$('input[name=pr_start]').val("${fn:substring(project.pr_start,0,10)}");
		$('input[name=pr_end]').val("${fn:substring(project.pr_end,0,10)}");
		$('select[name=pr_meeting]').val("${project.pr_meeting}");
		
		$('input[name=pr_support]:input[value="${project.pr_support}"]').attr('checked',true);
		$('input[name=mtype_num]:input[value="${project.mtype_num}"]').attr('checked',true);
		...
		$('#projectForm').submit(function(){
			...
			$('input[name=pr_title]').val();
			$('#category').val();
			
			
			$('input[name=pr_ex_period]').val();
			$('input[name=pr_payment]').val();
			$('input[name=pr_type]').val($('input[name=pr_type]:checked').val());
			
			$('input[name=pr_add]').val();
			
			$('select[name=pr_meeting]').val();
			
			$('input[name=pr_support]').val();
			$('input[name=mtype_num]').val();
...
<nav class="breadcrumb bg-white push"> 
	<a class="breadcrumb-item" href="#">프로젝트 임시저장</a> 
</nav>
		...
		<div class="block  ">      
			<div class="block-content  justify-content-center in row">    
				<div class="project-register-content">  
					<br>  
					<form  
						action="${pageContext.request.contextPath}/project/updateProject.do"  
						...
						<div class="row">    
							...
							<select class="form-control col-md-2" id="category">
								<c:if test="${empty category.cate_detail }">
								<option selected="selected" disabled="disabled" >카테고리</option>
								</c:if>
								<c:if test="${not empty category.cate_detail }">
								...
								<c:if test="${not empty category.cate_detail }">
								<option disabled="disabled" >세부 카테고리</option>
								<option selected="selected" value="${category.cate_num}">${category.cate_detail}</option>
								</c:if>
							</select>
						...
						<div class="form-group row">
							...
							<div class="col-5">
								<div class="custom-control custom-checkbox mb-5">  
									<input class="custom-control-input cc" type="radio" name="pr_type"
										id="example-checkbox1" value="0" > <label
						...
						<div class="row">
							...
							<textarea name="pr_content" id="pr_content" rows="20" cols="80">${project.pr_content }</textarea>
						...
						<div class="row ">
							...
							<div id="techDiv" class="tagsinput" style="width: 750px;  min-height:34px; border: 1px dotted gray; background-color: #F7F7F7;">
							<c:forEach items="${techlist}" var="tech"> 
								<span style='margin-right:10px; margin-bottom:10px;' class='tag' id='${tech.LAN_NAME }'><span>${tech.LAN_NAME }&nbsp;&nbsp;</span>
								 <a href='#' onclick='rem(${tech.LAN_NAME });' title='Removing tag'>x</a> 
								 </span>
								 <input type="hidden" name="technique" value="${tech.LAN_NAME }">
							</c:forEach>
							</div>
						</div>
						<br>
